...
bit_t timer_state(gbtimer_t *timer)
{
    ...
    if (bit_1_tac)
        bit_main_counter = bit_0_tac ? bit_get_16(timer->counter, 7) : bit_get_16(timer->counter, 5);
    else
        bit_main_counter = bit_0_tac ? bit_get_16(timer->counter, 3) : bit_get_16(timer->counter, 9);

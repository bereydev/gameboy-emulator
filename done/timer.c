...
bit_t timer_state(gbtimer_t* timer) {
	...
	data_t tac = cpu_read_at_idx((const)timer->cpu, REG_TAC);
...
void timer_incr_if_state_change(gbtimer_t* timer, bit_t old_state){
	if(old_state && !timer_state(timer)) {
		data_t second_timer_value = cpu_read_at_idx((const)timer->cpu, REG_TIMA);
		if(second_timer_value == 0xFF) {
			cpu_request_interrupt(timer->cpu, TIMER);
			second_timer_value = cpu_read_at_idx((const)timer->cpu, REG_TMA);
		}else {
			second_timer_value++;
			}
